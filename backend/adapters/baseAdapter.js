...
const isAllowedSite = (url = '', site = '') => {
  ...
  // Check direct slugs
  if (ALLOWED_SLUGS.includes(cleanSite)) return true;

  // Check direct domains
  for (const domain of ALLOWED_DOMAINS) {
    if (cleanUrl.includes(domain)) return true;
  }

  // Check Shopify indicators
  if (cleanUrl.includes('myshopify.com') || 
      (cleanUrl.includes('/products/') && cleanUrl.includes('/collections/')) ||
      cleanUrl.includes('cdn.shopify.com')) {
    return true;
  }
